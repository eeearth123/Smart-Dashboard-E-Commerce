...
# ==========================================
# PAGE 1: 📊 Executive Summary (Revised: Focus on Money)
# ==========================================
if page == "1. 📊 Executive Summary":
    ...
    with c2:
        ...
        if not df_display.empty:
            ...
            # ✅ แก้ไข Logic: ไม่ดูวันที่แล้ว แต่ดู "เงิน" แทน
            status_stats = df_display.groupby('status').agg({
                'customer_unique_id': 'count',
                'payment_value': 'sum'  # รวมยอดเงิน
            }).reset_index()
            ...
            donut = alt.Chart(status_stats).mark_arc(innerRadius=60).encode(
                theta=alt.Theta("Count", type="quantitative"), # ขนาดวงยังคงเป็นจำนวนคน (Volume)
                color=alt.Color("Status", scale=alt.Scale(domain=domain, range=range_), legend=dict(orient='bottom')),
                # ✅ Tooltip ใหม่: โชว์จำนวนคน และ ยอดเงิน!
                tooltip=[
                    'Status', 
                    alt.Tooltip('Count', format=',', title='จำนวนลูกค้า (คน)'),
                    alt.Tooltip('Total_Revenue', format=',.0f', title='ยอดเงินรวม (R$)')
                ]
            ).properties(height=350)
