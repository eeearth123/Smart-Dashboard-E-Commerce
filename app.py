...
# ==============================================================================
# PAGE 3: 🎯 Smart Action Plan (Redesigned for Better UX)
# ==============================================================================
    ...
    # ---------------------------------------------------------
    # 0. FILTER SECTION (ตัวกรองหมวดสินค้า)
    # ---------------------------------------------------------
    with st.container():
        c1, c2 = st.columns([2, 1])
        with c1:
            all_cats = list(df['product_category_name'].unique()) if 'product_category_name' in df.columns else []
            sel_cat = st.selectbox("📦 เลือกหมวดสินค้าเพื่อโฟกัสปัญหา:", ["รวมทุกหมวด (All Categories)"] + sorted(all_cats))
        
        # กรองข้อมูล
        if sel_cat != "รวมทุกหมวด (All Categories)":
            df_display = df[df['product_category_name'] == sel_cat].copy()
        else:
            df_display = df.copy()

        # สรุปภาพรวมประชากร
        total_pop = len(df_display)
        avg_ltv = df_display['payment_value'].mean() if 'payment_value' in df_display.columns else 150
        
        with c2:
            st.metric("👥 ลูกค้าทั้งหมดในกลุ่มนี้", f"{total_pop:,} คน", help="จำนวนลูกค้าตาม Filter ที่เลือก")

    st.markdown("---")

    # ---------------------------------------------------------
    # 1. STRATEGY TABS (แยกแท็บเพื่อให้ดูง่าย เล่าเรื่องทีละเรื่อง)
    # ---------------------------------------------------------
    # สร้าง Tab แยกตามปัญหา 4 ด้าน
    tab1, tab2, tab3, tab4 = st.tabs([
        "💳 1. แก้ปัญหาการจ่ายเงิน (Payment)", 
        "🚚 2. แก้ปัญหาค่าส่งแพง (Shipping)", 
        "❤️ 3. ง้อลูกค้าของช้า (Recovery)",
        "🛍️ 4. ขายพ่วงลดเสี่ยง (Bundling)"
    ])

    # ฟังก์ชันสำหรับวาด "Story" ในแต่ละ Tab (จะได้ไม่ต้องเขียนโค้ดซ้ำ)
    def render_strategy_story(title, icon, target_df, total_pop, strategy_name, default_cost, default_lift, rec_text):
        n_target = len(target_df)
        pct_problem = (n_target / total_pop) * 100 if total_pop > 0 else 0
        
        # 1. ส่วนแสดงปัญหา (Problem Statement)
        ...
        with c_prob:
            ...
            st.progress(pct_problem / 100)
            st.caption("แถบสีฟ้าคือสัดส่วนคนที่มีปัญหานี้เทียบกับลูกค้าทั้งหมด")

        # 2. ส่วนกำหนดงบประมาณ (Solution & Cost)
        with c_sol:
            st.markdown(f"**🛠️ วิธีแก้ไข: {strategy_name}**")
            st.write(rec_text)
            
            st.markdown("---")
            st.markdown("**🎚️ ปรับตัวแปร (Simulator):**")
            cost = st.number_input(f"งบต่อหัว ({icon})", value=default_cost, min_value=1, step=1, key=f"cost_{title}")
            lift = st.slider(f"คาดการณ์ผลสำเร็จ (Success Rate %)", 1, 50, default_lift, key=f"lift_{title}")

        # 3. ส่วนผลลัพธ์ (Result & ROI)
        with c_res:
            # คำนวณ ROI
            ...
            st.metric("💸 งบที่ต้องใช้ (Budget)", f"R$ {budget:,.0f}")
            st.metric("👥 ดึงลูกค้ากลับมาได้", f"{saved_users:,} คน")
            st.metric("💰 กำไร (ROI)", f"{roi:+.0f}%", delta=f"รายได้เพิ่ม R$ {revenue:,.0f}")

            if roi > 0:
                st.caption("✅ **คุ้มค่า!** ควรเริ่มแคมเปญนี้")
            else:
                st.caption("⚠️ **เสี่ยงขาดทุน** ลองลดงบต่อหัวดู")

    # --- TAB 1: PAYMENT STRATEGY ---
    with tab1:
        if 'payment_type' in df_display.columns:
            target = df_display[df_display['payment_type'].isin(['credit_card', 'boleto'])]
            render_strategy_story(
                "กลุ่มเสี่ยงจากการจ่ายเงิน", "💳", target, total_pop,
                "เปลี่ยน Cash เป็น Voucher", 
                default_cost=20, default_lift=10,
                rec_text="คนจ่ายบัตรเครดิต/โอน มี Churn สูงถึง 80% แต่คนใช้ Voucher Churn ต่ำมาก\n\n👉 **Action:** เสนอ Cashback 5% เข้า Wallet เพื่อเปลี่ยนพฤติกรรม"
            )

    # --- TAB 2: SHIPPING STRATEGY ---
    with tab2:
        if 'freight_ratio' in df_display.columns:
            target = df_display[df_display['freight_ratio'] > 0.2]
            avg_freight = target['freight_value'].mean() if not target.empty else 20
            render_strategy_story(
                "กลุ่มค่าส่งแพงเกินรับไหว", "🚚", target, total_pop,
                "ช่วยออกค่าส่ง (Free Shipping)", 
                default_cost=int(avg_freight), default_lift=15,
                rec_text=f"ลูกค้ากลุ่มนี้จ่ายค่าส่งแพงเกิน 20% ของราคาสินค้า (ค่าส่งเฉลี่ย R$ {avg_freight:.0f})\n\n👉 **Action:** แจกโค้ดส่วนลดค่าส่ง เพื่อลดแรงต้านในการซื้อ"
            )

    # --- TAB 3: RECOVERY STRATEGY ---
    with tab3:
        if 'delay_days' in df_display.columns:
            target = df_display[df_display['delay_days'] > 0]
            render_strategy_story(
                "กลุ่มโดนเท (ของส่งช้า)", "❤️", target, total_pop,
                "SMS ขอโทษ + คูปอง", 
                default_cost=15, default_lift=20,
                rec_text="ลูกค้ากลุ่มนี้ได้รับของช้ากว่ากำหนด ซึ่งสร้างความโกรธได้ง่าย\n\n👉 **Action:** ส่ง SMS ขอโทษทันที พร้อมให้ส่วนลดพิเศษในรอบหน้า"
            )

    # --- TAB 4: BUNDLING STRATEGY ---
    with tab4:
        if 'cat_churn_risk' in df_display.columns:
            target = df_display[df_display['cat_churn_risk'] > 0.8]
            render_strategy_story(
                "กลุ่มซื้อสินค้าความเสี่ยงสูง", "🛍️", target, total_pop,
                "Cross-sell สินค้าซื้อง่าย", 
                default_cost=10, default_lift=5,
                rec_text="ลูกค้าซื้อสินค้าที่ปกติคนไม่ค่อยกลับมาซื้อซ้ำ (เช่น เฟอร์นิเจอร์)\n\n👉 **Action:** ยิงแอดขายพ่วงสินค้าหมวดของใช้ในบ้าน (Housewares) ที่มีรอบการซื้อถี่กว่า"
            )
# ==========================================
