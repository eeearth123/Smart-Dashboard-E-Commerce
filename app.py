...
# ==============================================================================
    ...
    # ---------------------------------------------------------
    # 2. STRATEGY TABS (แสดงผลแยกตามหมวด)
    # ---------------------------------------------------------
    ...
    # --- TAB 1: PAYMENT STRATEGY (Voucher) ---
    with tab1:
        if 'payment_type' in df_p3.columns:
            # กลุ่มเป้าหมาย: จ่ายบัตรเครดิต/โอน (เสี่ยงสูง)
            target = df_p3[df_p3['payment_type'].isin(['credit_card', 'boleto'])]
            
            render_strategy_story(
                title="กลุ่มเสี่ยงจากการจ่ายเงิน (Payment Risk)", 
                icon="💳", 
                target_df=target, 
                total_pop=total_pop,
                strategy_name="เปลี่ยน Cash เป็น Voucher", 
                default_cost=20, 
                # AI Comparison logic
                compare_col='payment_type', 
                good_value=['voucher'],               # เทียบกับคนใช้ Voucher (Benchmark)
                bad_values=['credit_card', 'boleto'], # เทียบกับกลุ่มเป้าหมาย
                rec_text="ลูกค้าที่จ่ายบัตรเครดิต/โอน มีโอกาส Churn สูงมาก ต่างจากคนถือ Voucher ที่มักกลับมาซื้อซ้ำ\n\n👉 **Action:** เสนอ Cashback 5% เข้า Wallet เพื่อเปลี่ยนพฤติกรรม"
            )
        else:
            st.error("ไม่พบข้อมูล 'payment_type'")

    # --- TAB 2: SHIPPING STRATEGY (Freight) ---
    with tab2:
        if 'freight_ratio' in df_p3.columns:
            # กลุ่มเป้าหมาย: ค่าส่งแพงเกิน 20%
            target = df_p3[df_p3['freight_ratio'] > 0.2]
            avg_freight = target['freight_value'].mean() if not target.empty else 20
            
            render_strategy_story(
                title="กลุ่มค่าส่งแพงเกินรับไหว (Freight Pain)", 
                icon="🚚", 
                target_df=target, 
                total_pop=total_pop,
                strategy_name="ช่วยออกค่าส่ง (Free Shipping)", 
                default_cost=int(avg_freight), 
                # AI Comparison logic
    ...
    # --- TAB 3: RECOVERY STRATEGY (Delay) ---
    with tab3:
        if 'delay_days' in df_p3.columns:
            # กลุ่มเป้าหมาย: ของส่งช้า
            target = df_p3[df_p3['delay_days'] > 0]
            
            render_strategy_story(
                title="กลุ่มโดนเท/ของส่งช้า (Delay Recovery)", 
                icon="❤️", 
                target_df=target, 
                total_pop=total_pop,
                strategy_name="SMS ขอโทษ + คูปอง", 
                default_cost=15, 
                # AI Comparison logic
                compare_col='delay_days',
                good_value=0,    # เทียบกับคนที่ได้ของตรงเวลา (Delay = 0)
                bad_values=None,
                rec_text="ความล่าช้าทำลายความเชื่อมั่น และเป็นสาเหตุหลักของการเปลี่ยนใจ\n\n👉 **Action:** ส่ง SMS ขอโทษทันทีที่รู้ว่าของช้า พร้อมแนบส่วนลดพิเศษ"
            )
        else:
            st.error("ไม่พบข้อมูล 'delay_days'")

    # --- TAB 4: BUNDLING STRATEGY (Category Risk) ---
    with tab4:
        if 'cat_churn_risk' in df_p3.columns:
            # กลุ่มเป้าหมาย: ซื้อของในหมวดเสี่ยงสูง
            target = df_p3[df_p3['cat_churn_risk'] > 0.8]
            
            render_strategy_story(
                title="กลุ่มซื้อสินค้าความเสี่ยงสูง (High Risk Category)", 
                icon="🛍️", 
                target_df=target, 
                total_pop=total_pop,
                strategy_name="Cross-sell สินค้าซื้อง่าย", 
                default_cost=10, 
                # AI Comparison logic
                compare_col='cat_churn_risk',
                good_value=0.5,  # เทียบกับหมวดสินค้าที่คนติดงอมแงม (Risk ต่ำ)
                bad_values=None,
                rec_text="สินค้าบางหมวดคนซื้อครั้งเดียวจบ (เช่น เฟอร์นิเจอร์) ทำให้ Churn สูง\n\n👉 **Action:** ยิงแอดขายพ่วงสินค้าหมวดของใช้ในบ้าน (Housewares) ที่ต้องซื้อซ้ำบ่อยๆ"
            )
        else:
