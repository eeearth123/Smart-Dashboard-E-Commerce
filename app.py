...
# ==========================================
    ...
    with st.expander("ℹ️ AI คำนวณความคุ้มค่า (ROI) และจัดสรรงบอย่างไร?"):
        st.markdown("""
        **1. การคำนวณกำไรคาดหวัง (Expected Profit):**
        > สูตร: `(จำนวนคน × โอกาสสำเร็จ × ยอดซื้อเฉลี่ยต่อหัว LTV) - ต้นทุนแคมเปญ`
        * ระบบจะประเมินว่าเงินที่เราลงทุนไป (ต้นทุนแคมเปญ) จะสามารถดึงลูกค้ากลับมาซื้อของ (ยอดซื้อเฉลี่ย) ได้เท่าไหร่ 
        * หากตัวเลข **ติดลบ** แปลว่าแคมเปญนี้ได้ไม่คุ้มเสีย (ขาดทุน)

        **2. การคำนวณความคุ้มค่า (ROI - Return on Investment):**
        > สูตร: `(กำไรคาดหวัง ÷ ต้นทุนแคมเปญ) × 100`
        * ตัวเลขนี้บอกว่า ลงทุนไป 100 บาท จะได้กำไรกลับมากี่บาท (ยิ่งเปอร์เซ็นต์สูง ยิ่งคุ้มค่า)

        **3. วิธีการจัดสรรงบ (Allocation Logic):**
        * ระบบใช้หลักการ **Greedy Algorithm** โดย AI จะนำแคมเปญทั้งหมดมาเรียงลำดับจาก ROI สูงสุดไปต่ำสุด
        * จากนั้นระบบจะเริ่ม "เทงบประมาณที่มี" ให้กับแคมเปญที่คุ้มค่าที่สุดเป็นอันดับ 1 ก่อน ถ้างบยังเหลือจึงจะจัดสรรให้แคมเปญอันดับรองลงมา จนกว่าเงินจะหมด
        """)
