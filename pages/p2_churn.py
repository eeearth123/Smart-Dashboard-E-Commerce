# ============================================================
# pages/p2_churn.py — Churn Overview (v5)
# ============================================================
    ...
    with st.expander("🌪️ กรองข้อมูล", expanded=False):
        ...
        with fa:
            sel_cats = st.multiselect(
                "หมวดสินค้า (ว่าง = ทั้งหมด):", safe_cats(df), key="p2_cat"
            )
        with fb:
...
| ⚠️ Monitor | AI ≤ threshold **แต่** Late > 1.5x | rule เห็น AI ยังให้โอกาส |
    ...
    with col_left:
        ...
        st.caption("กลุ่ม **Early Warning** คือ value ของโมเดล — จับได้ก่อน rule")
        ...
        labels = list(MATRIX_GROUPS.values())

        for col, data, (tag, bg, color, action), label in zip(grid, cells, CELL_STYLE, labels):
            with col:
                st.markdown(
                    f"""<div style="background:{bg};border-radius:10px;padding:12px;margin:4px 0">
    ...
    with col_right:
        ...
        st.caption("หมวดไหนมีสีแดง/ส้มเยอะ = ลูกค้าหนีสูง")
        if "product_category_name" in df_d.columns:
            _render_category_chart(df_d)

    # ── พฤติกรรมแต่ละกลุ่ม (มี KPI อยู่ข้างใน) ──────────────
    st.markdown("---")
...
def _render_category_chart(df_d):
    ...
    cat_stat  = (
        ...
        .size().reindex(idx, fill_value=0).reset_index(name="count")
    )
    ...
    st.altair_chart(
        alt.Chart(cat_stat).mark_bar().encode(
    ...
    st.caption(f"💡 แสดงทุกหมวด ({len(all_cats)} หมวด) เรียงจาก High Risk มากสุด")


def _render_behaviour(grp: pd.DataFrame, total_all: int):
    if grp.empty:
    ...
    # ส่งช้ากว่ากำหนด = delay_days > 0 (delivered > estimated)
    # กรอง NaN ออกก่อน (แถวที่ยังไม่ได้รับของ)
    if "delay_days" in grp.columns:
        delivered_rows = grp["delay_days"].notna()
        late_pct = (grp.loc[delivered_rows, "delay_days"] > 0).mean() * 100 \
                   if delivered_rows.sum() > 0 else 0.0
    else:
        late_pct = None
    ...
    b3.metric("⏰ ส่งช้ากว่ากำหนด",
              ...
              delta_color="inverse",
              help="คิดจากออเดอร์ที่ส่งแล้วเท่านั้น (delivered > estimated)")
    ...
    with c_cat:
        if "product_category_name" in grp.columns and "price" in grp.columns:
            cat_price = (
                ...
                .agg(orders=("price","count"), avg_price=("price","mean"))
                .reset_index()
    ...
    with c_pay:
        if "payment_type" in grp.columns:
            ...
            st.altair_chart(
                alt.Chart(pay_dist).mark_arc(innerRadius=40).encode(
                    theta=alt.Theta("สัดส่วน (%):Q"),
                    color=alt.Color("วิธีจ่าย:N",
